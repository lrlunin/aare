...
template <typename FinderType, bool update_pedestal>
void run_benchmark(benchmark::State &state, const char *label) {
    ...
    for (auto _ : state) {
        ...
        for (aare::Frame &frame : data.v_test_frames) {
            finder.find_clusters(frame.view<uint16_t>(), 0, update_pedestal);
...
void BM_ClusterFinder(benchmark::State &state) {
    run_benchmark<Finder, true>(state, "find_clusters");
}

void BM_ClusterFinder_SkipPedestals(benchmark::State &state) {
    run_benchmark<Finder, false>(state, "find_clusters");
}
} // namespace

// Register both benchmarks
BENCHMARK(BM_ClusterFinder)->Unit(benchmark::kMillisecond);
BENCHMARK(BM_ClusterFinder_SkipPedestals)->Unit(benchmark::kMillisecond);